...
const HotelReservationById = ({ userSession, id }) => {
    ...
    const {
        ...
        error,
        refetch,
    } = useQuery(HOTEL_ROOM_RESERVATION_BY_ID, {
        variables: {
            id,
        },
    });

    const [approveReservation] = useMutation(APPROVE_HOTEL_ROOM_RESERVATION);
    const [denyReservation] = useMutation(DENY_HOTEL_ROOM_RESERVATION);
    ...
    const { hotelRoomReservationById } = data;
    const {
        reservationId,
        fromDateTime,
        toDateTime,
...
export default HotelReservationById;
